...
class Receipt:
    def __init__(self, name, items, prices, quantities):
        self.name = name
        self.items = items
        self.prices = prices
...
class GUI(Receipt):
    def __init__(self, name, items, prices, quantities, master):
        super().__init__(name, items, prices, quantities)
        self.master = master
    ...
    def get_receipt_text(self):
        receipt = f"Customer Name: {self.name}\n\n"
        receipt += "Items Purchased:\n"
        receipt += f"{'Item':<20} {'Price':<10} {'Quantity':<10} {'Subtotal':<10}\n"
        receipt += '-' * 60 + '\n'
        for item, price, quantity in zip(self.items, self.prices, self.quantities):
            ...
            receipt += f"{item.strip():<20} ${price:<10.2f} {quantity:<10} ${subtotal:<10.2f}\n"
        receipt += '-' * 60 + '\n'
        receipt += f"{'Total Amount:':<40} ${sum([price * quantity for price, quantity in zip(self.prices, self.quantities)]):.2f}"
        return receipt
...
class ReceiptMaker:
    def __init__(self, master):
        ...
        self.label_items = tk.Label(master, text="Items (separated by comma(,)):")
        self.label_items.pack()
        self.entry_items = tk.Entry(master)
        self.entry_items.pack()

        self.label_prices = tk.Label(master, text="Prices (separated by comma(,)):")
        self.label_prices.pack()
        self.entry_prices = tk.Entry(master)
        self.entry_prices.pack()

        self.label_quantities = tk.Label(master, text="Quantities (separated by comma(,)):")
    ...
    def print_receipt(self):
        ...
        receipt = GUI(name, items, prices, quantities, self.master)
